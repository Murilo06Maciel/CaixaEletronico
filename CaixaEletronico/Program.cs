...
public class Program
{
	public class Conta
	{
		...
		public Conta(string nome, string agencia, int numero, double saldo, string senha)
		{
			Nome = nome;
			Agencia = agencia;
			Numero = numero;
		...
		switch (opcao)
		{
			case 1:
				...
				else
				{
					...
					{
						contas.Add(new Conta(nome, agencia, numero, saldoContaCorrente, senha));
			...
			case 5:
				...
				if (double.TryParse(Console.ReadLine(), out transferencia) && transferencia > 0 && transferencia <= saldoContaCorrente)
				{
					...
					Conta contaDestinatario = contas.Find(c => c.Nome.Equals(destinatario, StringComparison.OrdinalIgnoreCase) && c.Agencia.Equals(agenciaR, StringComparison.OrdinalIgnoreCase));
					if (contaDestinatario != null)
					{
						...
						if (valorComTaxa <= saldoContaCorrente)
						{
							...
							extrato.Add($"Transferência de {transferencia:C} para {destinatario} (Remetente: {remetente}) (Taxa: {transferencia * taxaTransferencia:C})");
					...
					{
						Console.WriteLine("Transferência não realizada! Destinatário não encontrado.");
					}
				}
				else
				{
			...
			case 6:
				...
				if (double.TryParse(Console.ReadLine(), out double poupancaValor) && poupancaValor > 0)
				{
					...
					if (int.TryParse(Console.ReadLine(), out int meses) && meses > 0)
					{
						...
						Console.WriteLine($"Depósito realizado com sucesso na Poupança. Saldo final da Poupança é de: {saldoPoupanca:C}");
					}
					else
					{
			...
			case 7:
				...
				{
					Console.Write("Digite quantos meses você deseja investir no CDB: ");
					if (int.TryParse(Console.ReadLine(), out int mesesCDB) && mesesCDB > 0)
					{
						double taxaCDB = 0.15;
						double rendimentoCDB = cdbValor * taxaCDB * mesesCDB / 12;
						saldoContaCorrente += cdbValor + rendimentoCDB;
						extrato.Add($"Investimento em CDB de {cdbValor:C} com rendimento de {rendimentoCDB:C} após {mesesCDB} meses");
						Console.WriteLine($"Investimento realizado com sucesso em CDB. Saldo final é de: {saldoContaCorrente:C}");
					}
					else
					{
						Console.WriteLine("Número de meses inválido!");
